...
import { loginWithProviderThunk, signupThunk, signupWithProviderThunk } from '@/modules/redux/slices/auth/thunk/auth.thunk';
...
import { RegisterData } from '../types/auth.interfaces';
...
const SocialAuthButtons: React.FC<SocialAuthButtonsProps> = ({ type }) => {
  ...
  const loginWithGoogle = async () => {
    ...
    try {
      if (type === 'signup') {
        await dispatch(signupWithProviderThunk()).unwrap()
      } else {
        await dispatch(loginWithProviderThunk()).unwrap()
      }

      router.push("/dashboard")
    } catch (error: any) {
      console.log("🚀 ~ SocialAuthButtons.tsx:26 ~ loginWithGoogle ~ error:", error)
    }
